...
for total_unstitched=total_number_img:-1:2
    ...
    for ii=1:total_iteration
        left_img_rgb = img_pool{shuffler(ii,1)};
        right_img_rgb = img_pool{shuffler(ii,2)};
        [stiched_img_pool{ii}, inliers, residual] = stiching(left_img_rgb, right_img_rgb, harris_coe, ransac_coef, neighbor_size,putative_matches_limit,SHOW_DETAILS);
