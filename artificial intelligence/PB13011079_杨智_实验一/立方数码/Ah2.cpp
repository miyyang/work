...
void try_all_move(State* s)
{
    ...
    if ((space_z <= 1) && (current[(space_x * 3 + space_y) * 3 + space_z + 1] > OFFSET))
    {
        ...
        swap(val, val + 1);
        temp->h = s->h + ((goal_place[*val - OFFSET][2] <= space_z)? -1 : 1);
    ...
    if ((space_y > 0) && (current[(space_x * 3 + space_y - 1) * 3 + space_z] > OFFSET))
    {
        ...
        char* val = temp->state + (space_x * 3 + space_y) * 3 + space_z;
        swap(val, val - 3);
        temp->h = s->h + ((goal_place[*val - OFFSET][1] >= space_y)? -1 : 1);
        Successors.push_back(temp);
    }
    // Right ok
    ...
    if ((space_x <= 1) && (current[(space_x * 3 + 3 + space_y) * 3 + space_z] > OFFSET))
    {
        ...
        char* val = temp->state + (space_x * 3 + space_y) * 3 + space_z;
        swap(val, val + 9);
        temp->h = s->h + ((goal_place[*val - OFFSET][0] <= space_x)? -1 : 1);
        Successors.push_back(temp);
    }
    // Back
...
int Compute_H(State* s)
{
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            for (k = 0; k < 3; k++)
            {
                val = a[(i * 3 + j) * 3 + k] - OFFSET;
                if (val > 0)
                {
                    char* c = goal_place[val];
                    int temp = (abs(c[0] - i) + abs(c[1] - j) + abs(c[2] - k));
                    result += temp;
                }
            }
    return result;
...
int Astar()
{
    ...
    start.g = 0;
    start.h = start.f = Compute_H(&start);
